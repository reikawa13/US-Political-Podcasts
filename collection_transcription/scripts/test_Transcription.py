...
def getPathToWAV(podName, epNum, audio_file):
  local_dir = os.path.join(os.getcwd(), 'temp_audio')
  if not os.path.exists(local_dir):
    os.makedirs(local_dir)
  local_file_path = os.path.join(local_dir, audio_file)

  # Remote path on the remote machine
  remote_dir = "/local/converted_audio"  # Adjust the path as needed
...
def getPathToMP3(row):
  ...
  # Remote path on the remote machine
  remote_dir = "/local/Audio-Files2"  # Adjust the path as needed
...
def main(): 
  ...
  """
#  csvName = "podMetadata2.csv"
  csvName = "conversion.csv"
  ...
  with open(csvName, 'r') as csvfile: 
    ...
    for row in datareader:
      ...
      try:
        ...
        audioPath = getPathToWAV(podName, epNum, audio_file)
        print("path is: %s" % audioPath)
      except:
