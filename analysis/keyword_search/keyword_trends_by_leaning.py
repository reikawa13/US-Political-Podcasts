#!/usr/bin/env python3
"""Aggregate keyword counts over time by political leaning and plot trends."""
# Usage: python3 analysis/keyword_trends_by_leaning.py --freq W
...
REFERENCE_PERIODS = {
    "W": [
        "2023-05-08",  # week including 2023-05-11
        "2024-02-05",  # early February 2024
        "2024-02-19",  # week including 2024-02-20
        "2024-06-17",  # week including 2024-06-18
        "2024-09-09",  # week including 2024-09-10
        "2024-10-07",  # week including 2024-10-11
    ],
}


def parse_date(value: str) -> datetime:
    # Try RFC 822 first, then common ISO-like fallbacks.
    ...
    with counts_path.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if end_date is not None and dt > end_date:
                continue
            try:
                count = int(float(row.get("total_keyword_hits", 0) or 0))
            except ValueError:
                count = 0
...
def write_timeseries(path: Path, totals: dict[str, dict[datetime, int]], periods: list[datetime], freq: str) -> None:
...
def plot_timeseries(
    ...
    try:
        ...
        print("matplotlib is required to render the plot. Install it or use --no-plot.", file=sys.stderr)
        raise
    ...
    for leaning, color in [("Liberal", "#1f77b4"), ("Moderate", "#2ca02c"), ("Conservative", "#d62728")]:
        y = [totals.get(leaning, {}).get(p, 0) for p in periods]
        plt.plot(x, y, label=leaning, color=color, linewidth=1.3, alpha=0.45)
        if moving_average_window > 1:
            ma = moving_average(y, moving_average_window)
            plt.plot(
                x,
                ma,
                label=f"{leaning} ({moving_average_window}-period MA)",
                color=color,
                linewidth=2.6,
            )

    plt.title("Keyword Counts Over Time by Political Leaning")
    x_label = {"D": "Day", "W": "Week", "M": "Month", "Q": "Quarter", "Y": "Year"}.get(freq, "Period")
    plt.xlabel(x_label)
    ...
    if x_labels:
        step = 3 if len(x_labels) >= 3 else 1
        ...
        ax.set_xticks(list(range(len(x_labels))), minor=True)
    reference_labels = REFERENCE_PERIODS.get(freq, [])
    x_lookup = {label: index for index, label in enumerate(x_labels)}
    for label in reference_labels:
        if label in x_lookup:
            ax.axvline(x_lookup[label], color="#ffd700", linewidth=1.4, alpha=0.95)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate keyword counts by political leaning over time.")
    parser.add_argument(
        "--counts",
        default="analysis/keyword_counts_by_episode_overall.csv",
        help="Keyword counts CSV",
    )
    parser.add_argument("--freq", default="W", choices=["D", "W", "M", "Q", "Y"], help="Aggregation frequency")
    parser.add_argument(
        "--start-date",
        default="2023-01-01",
    ...
    parser.add_argument(
        "--end-date",
        default="2024-11-05",
        help="Latest date to include (YYYY-MM-DD). Use an empty value to disable.",
    )
    parser.add_argument("--metadata", default="podMetadata_Nov5_with_leaning.csv", help="Episode metadata CSV (for dates)")
    parser.add_argument(
        "--out-csv",
        default="analysis/keyword_counts_by_leaning_timeseries.csv",
        help="Output CSV path",
    )
    parser.add_argument(
        "--out-plot",
        default="analysis/keyword_counts_by_leaning_trend.png",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Moving-average window to overlay on the plot (1 disables smoothing).",
    )
    ...
    try:
        ...
        totals, period_set = aggregate_counts(counts_path, meta_dates, args.freq, start_date, end_date)
    ...
    if not periods:
        print("No data matched between counts, metadata, and top100 lists.", file=sys.stderr)
        return 1
    ...
    if not args.no_plot:
        ...
        plot_timeseries(out_plot, totals, periods, args.freq, args.moving_average_window)
