#!/usr/bin/env python3
"""Aggregate migrant crime counts over time by political leaning (weekly)."""
# Usage: python3 analysis/migrant_crime/migrant_crime_trends_by_leaning_weekly.py --start-date 2023-01-01
...
def write_timeseries(
    path: Path, totals: dict[str, dict[datetime, int]], periods: list[datetime]
) -> None:
...
def plot_timeseries(
    ...
    x_labels = [format_period(p) for p in periods]
    ...
    plt.title("Migrant Crime Phrase Counts Over Time by Political Leaning (Weekly)")
    plt.xlabel("Week")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Aggregate migrant crime counts by political leaning over time (weekly)."
    )
    parser.add_argument(
        "--counts",
        default="migrant_crime_counts_by_episode_overall.csv",
    ...
    parser.add_argument(
        "--out-csv",
        default="migrant_crime_counts_by_leaning_timeseries_weeklys.csv",
        help="Output CSV path",
    )
    parser.add_argument(
        "--out-plot",
        default="migrant_crime_counts_by_leaning_trend_weekly.png",
    ...
    write_timeseries(out_csv, totals, periods)

    if not args.no_plot:
        out_plot = Path(args.out_plot)
        plot_timeseries(out_plot, totals, periods)
